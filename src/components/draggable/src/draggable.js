 ...
 * @LastEditors: zhanxin
 * @LastEditTime: 2018-08-27 15:38:44
 * @Description: 拖拽方法
 */

let lastestInfo = null;
...
function buildAttribute(object, propName, value) {
  ...
  object[propName] = Object.keys(value).length === 0 ? lastestInfo : value;
  return object;
}
...
export default {
  ...
  render(h) {
    ...
    if (Object.keys(this.$attrs).length !== 0) {
      lastestInfo = JSON.parse(JSON.stringify(this.$attrs));
    }
  ...
  mounted() {
    ...
    // require.ensure(['sortablejs'], Sortable => {
    //   this._sortable = new Sortable(this.rootContainer, options);
    // });
    import(/* webpackChunkName: 'sortablejs.min'*/ 'sortablejs').then(Module => {
      this._sortable = new Module.default(this.rootContainer, options);
    });
    // this._sortable = new Sortable(this.rootContainer, options);
    //this.computeIndexes()
  },
